...
public class Utils {
    ...
    public static <T> T loadTpl(Class<T> clazz, Model model) {
        try {
            ...
            if (model == null) { // 모델 데이터가 없을 때
                Method method = clazz.getDeclaredMethod("print");  // print 함수 호출
                method.invoke(obj);
    ...
    public static <T> T loadController(Class<T> clazz, Model model) {
         ...
         */
        try {
            Object obj = BeanContainer.getBean(clazz); // 싱글톤 패턴

            // Controller 인 경우만 처리
            // obj 객체가 Controller 클래스에 속하는지
            if (obj instanceof Controller controller) {
                ...
                method.invoke(obj); // 동적 호출

                return (T) controller;
            }
        } catch (Exception e) {
            if (e instanceof InvocationTargetException targetException) { // 감싼 예외를 꺼냄
                Throwable throwable = targetException.getTargetException();
                if (throwable instanceof CommonException commonException) {
                    throw commonException;
                }
