 ...
 */
public abstract class Controller { // 추상 클래스
     ...
     * @return
     */
    protected String getPromptText() { // 상속 -> 유연하게 문구를 바꾸기 위해
    ...
    protected void process(String input) {
        if (inputProcess != null) {
            inputProcess.accept(input); // 인터페이스 구현내용 X 그때그때 정해줘야함
        }
    }
    ...
    public void prompt() {
        Utils.drawLine('-', 30);
        System.out.print(getPromptText()); // 메뉴선택문구 호출
        ...
        if (promptProcess == null) {
            ...
            promptProcess.run(); // 상품 등록
        }
    }

    /**
     * 콘솔 프로그램은 컨트롤러가 실행되는 절차가 고정되어 있으며 이는 변경이 불가 하다. 
     * 따라서 final로 하위 클래스가 재정의 하지 못하도록 고정합니다.(메서드 재정의 불가)
     * 실행 및 출력 순서는
     * 1) common() : 공통 화면
