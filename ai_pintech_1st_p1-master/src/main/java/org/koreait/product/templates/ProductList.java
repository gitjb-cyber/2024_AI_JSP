...
public class ProductList implements Template {
    ...
    @Override
    public void print() { // 조회된 상품 출력
        System.out.println("상품목록");
        Utils.drawLine('-', 30);
        if (items != null && !items.isEmpty()) {
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy.MM.dd");
            for (Product item : items) {
                System.out.printf("상품번호: %d / 상품명: %s / 판매가: %d원 / 재고: %d개 / 등록일: %s%n", item.getSeq(), item.getName(), item.getPrice(), item.getStock(), formatter.format(item.getRegDt()));
            }
            return;
        }
